...
tokens Cxx/yy. This is deliberately below object/property helper APIs: if the
literal token is absent from the source bytes, ProofGrid cannot infer it from
STB, Ortbeton, geometry, structural role, or common construction practice.
"""
...
def build(path:Path):
    ...
    model=ifcopenshell.open(str(path)); products=model.by_type("IfcProduct"); require(len(products)==1026,"accepted IfcProduct count mismatch")
    occurrences=[]
    ...
    record={
      ...
      "verdict":VERDICT,
      "source":{"sha256":SOURCE_SHA256,"file_bytes":SOURCE_BYTES,"ifc_product_count":1026},
      ...
      "method":{"scope":"ENTIRE_NATIVE_IFC_UTF8_SOURCE_TEXT","literal_regex":"C\\s*dd\\s*/\\s*dd","helper_api_inference_used":False,"inference_from_stb_or_material_names":False,"fuzzy_matching":False},
