...
class FoundryRequest:
    ...
    def from_dict(cls, raw: Mapping[str, Any]) -> "FoundryRequest":
        ...
        def enum_value(enum_type: type[_TextEnum], key: str, default: _TextEnum) -> _TextEnum:
        ...
        source = raw.get("source_material", "")
        request = cls(
            source_material=str(source),
            operation=enum_value(FoundryOperation, "operation", FoundryOperation.AUTO),
            ...
            completion_target=str(
                raw.get("completion_target", "PROMPT_PACKAGE_COMPLETE")
            ).strip()
        ...
        errors, _ = validate_request(request)
        if errors:
...
def _detect_operation(request: FoundryRequest, text: str) -> tuple[FoundryOperation, str]:
    ...
    return FoundryOperation.CREATE, "No transformation-specific rule matched; create a prompt from the request."


def _select_adapter(target_platform: str) -> str:
    platform = _normalize(target_platform)
    if any(token in platform for token in ("openai", "gpt-5", "gpt5", "reasoning")):
        return "openai-reasoning" if "openai" in platform or "gpt" in platform else "generic-reasoning"
    if any(token in platform for token in ("fast", "mini", "low-latency")):
        return "fast-model"
    if any(token in platform for token in ("local", "small model", "small-model")):
        return "local-small-model"
    return "generic-reasoning"


def _classify_risk(request: FoundryRequest, text: str, modules: Sequence[str]) -> str:
    safety_critical = (
    ...
    if request.task_mode is UnderlyingTaskMode.EXECUTE_CONSEQUENTIAL or _matches(text, high):
        return "HIGH"
    if request.confirmed_tools or "tooling" in modules or request.task_mode is not UnderlyingTaskMode.DO_NOT_EXECUTE:
        return "MODERATE"
    return "LOW"
...
def route_request(request: FoundryRequest) -> RoutingDecision:
    ...
    domain_rules: tuple[tuple[str, tuple[str, ...], str], ...] = (
        (
            "research",
            (r"\bresearch\b", r"\bprimary sources?\b", r"\bcitations?\b", r"\bliterature\b", r"\bprior art\b", r"\bverify current\b", r"\blatest\b"),
            "Research or current-fact controls are materially relevant.",
        ),
        (
            "software",
            (r"\bsoftware\b", r"\bcode\b", r"\bapi\b", r"\brepositor(?:y|ies)\b", r"\bcompiler\b", r"\bdatabase\b", r"\bcloud\b", r"\bdeploy(?:ment)?\b", r"\bweb app\b", r"\bmobile app\b"),
            "Software or computational-system requirements are present.",
        ),
        (
            "built_environment",
            (r"\bhouse\b", r"\bbuilding\b", r"\barchitecture\b", r"\bstructural\b", r"\bhvac\b", r"\bplumbing\b", r"\bconstruction\b", r"\bblueprint\b"),
            "Built-environment or construction coordination is required.",
        ),
        (
            "creative",
            (r"\bimage\b", r"\bvideo\b", r"\bfilm\b", r"\bstory\b", r"\bvisual design\b", r"\bbrand(?:ing)?\b", r"\banimation\b", r"\bcinematic\b"),
            "Creative-direction controls materially improve the deliverable.",
        ),
        (
            "business",
            (r"\bbusiness\b", r"\bmarket\b", r"\bcustomer\b", r"\brevenue\b", r"\bcommercial\b", r"\bfinance\b", r"\bproduct strategy\b"),
            "Business, product, or operating-model requirements are present.",
        ),
        (
            "education",
            (r"\bteach\b", r"\blesson\b", r"\bcurriculum\b", r"\btraining\b", r"\blearner\b", r"\bassessment\b"),
            "Educational sequencing and assessment are relevant.",
        ),
        (
            "innovation",
            (r"\bnext[- ]generation\b", r"\bbreakthrough\b", r"\bnovel\b", r"\bcategory leader\b", r"\bbetter than\b", r"\binnovation\b"),
            "The request makes an innovation or differentiation claim.",
        ),
        (
            "definitive_completion",
            (r"\bdefinitive completion\b", r"\bimplementation[- ]ready\b", r"\bfully finished\b", r"\bcomplete product\b", r"\bcompletion record\b", r"\bfinish the project\b"),
            "A finite endpoint and exact completion semantics are required.",
        ),
        (
            "security",
            (r"\bsecurity\b", r"\bprivacy\b", r"\bauthentication\b", r"\bauthori[sz]ation\b", r"\bprompt injection\b", r"\bsecret\b", r"\bcredential\b"),
...
def _build_pir(request: FoundryRequest, decision: RoutingDecision, source_hash: str) -> dict[str, Any]:
    permissions = {
        "read": request.task_mode in {
            UnderlyingTaskMode.EXECUTE_READ_ONLY,
            UnderlyingTaskMode.EXECUTE_REVERSIBLE,
            UnderlyingTaskMode.EXECUTE_CONSEQUENTIAL,
        },
        "draft": True,
        "write_reversible": request.task_mode in {
            UnderlyingTaskMode.EXECUTE_REVERSIBLE,
            UnderlyingTaskMode.EXECUTE_CONSEQUENTIAL,
        },
    ...
    if request.objective is None:
        assumptions.append("Derive the objective conservatively from explicit source material.")
    if request.deliverable is None:
        assumptions.append("Derive the deliverable from the requested speech act and output mode.")
    ...
    return {
        ...
        "underlying_project_state": "PROJECT_NOT_EXECUTED"
...
def _output_contract(output_mode: OutputMode) -> str:
    contracts = {
        OutputMode.PROMPT_ONLY: "Return only the final copy-ready prompt and any required variable block.",
        OutputMode.STANDARD: "Return interpreted objective, material assumptions/blockers, selected modules, final prompt, input template, and validation state.",
        OutputMode.FULL_FOUNDRY: "Return objective, operation, assumptions/blockers, intelligence stack, requirements summary, architecture decision, final prompt, reusable input template, output contract, validation cases, quality gates, compact variant, and version record.",
        OutputMode.AUDIT_REPORT: "Return executive verdict, strengths, defects by severity, contradictions, missing parameters, tool/authority/evidence/security findings, and prioritized remediation. Rewrite only when explicitly required.",
        OutputMode.DIFF_PATCH: "Return original fragment, revised fragment, reason, severity, behavioural effect, and complete final prompt.",
        OutputMode.TECHNICAL_SPECIFICATION: "Return the full Foundry package plus schemas, interfaces, states, preconditions, postconditions, invariants, errors, permission model, threat model, tests, deployment assumptions, observability, rollback, maintenance, and implementation notes.",
    }
...
def validate_request(request: FoundryRequest) -> tuple[list[str], list[str]]:
    ...
    unknown_actions = sorted(set(request.authorized_actions) - ALLOWED_CONSEQUENTIAL_ACTIONS)
    if unknown_actions:
        errors.append(f"unsupported authorized_actions: {', '.join(unknown_actions)}")
    prohibited_authorized = sorted(set(request.authorized_actions) & set(request.prohibited_actions))
    if prohibited_authorized:
        errors.append(
            "actions cannot be both authorized and prohibited: "
            + ", ".join(prohibited_authorized)
        )
    if request.task_mode is UnderlyingTaskMode.EXECUTE_CONSEQUENTIAL and not request.authorized_actions:
        errors.append(
            "EXECUTE_CONSEQUENTIAL requires at least one explicit authorized_action"
        )
    if request.task_mode is not UnderlyingTaskMode.EXECUTE_CONSEQUENTIAL and request.authorized_actions:
        warnings.append(
            "authorized_actions are recorded but cannot be exercised outside EXECUTE_CONSEQUENTIAL"
        )
    return errors, warnings


def validate_package(package: Mapping[str, Any], request: FoundryRequest) -> dict[str, Any]:
    errors: list[str] = []
    warnings: list[str] = []
    ...
    if request.task_mode is UnderlyingTaskMode.DO_NOT_EXECUTE and "DO_NOT_EXECUTE" not in runtime_prompt:
    ...
    if request.task_mode is UnderlyingTaskMode.EXECUTE_CONSEQUENTIAL:
        listed = set(package.get("pir", {}).get("permissions", {}).get("authorized_consequential_actions", []))
        if listed != set(request.authorized_actions):
            errors.append("compiled authorization differs from the explicit action list")
    ...
    return {
        ...
        "checks": {
            ...
            "source_boundary": not any("encoded source" in item for item in errors),
            ...
            "authority_ceiling": not any("authorization differs" in item for item in errors),
        },
    }
...
def compile_request(request: FoundryRequest) -> dict[str, Any]:
    ...
    missing_modules = [name for name in decision.modules if name not in resources["modules"]]
    if missing_modules:
...
FOUNDARY_OPERATION: {decision.operation.value}
...
"""
    ...
    if request.context_budget is not None and len(runtime_prompt) > request.context_budget:
        raise PromptOSError(
            f"compiled runtime prompt is {len(runtime_prompt)} characters, exceeding context_budget={request.context_budget}; reduce modules or raise the budget"
        )
    ...
    if package["validation"]["status"] != "PASS":
        raise PromptOSError("Compiled package failed validation: " + "; ".join(package["validation"]["errors"]))
    return package
...
def generate_corpus() -> list[dict[str, Any]]:
    ...
    if len(cases) != 120:
        raise AssertionError(f"Corpus generator produced {len(cases)} cases, expected 120")
    return cases


def split_corpus(cases: Sequence[dict[str, Any]] | None = None) -> dict[str, list[dict[str, Any]]]:
...
def run_conformance(cases: Sequence[dict[str, Any]] | None = None) -> dict[str, Any]:
    material = list(cases if cases is not None else generate_corpus())
    ...
    for case in material:
        ...
        missing = sorted(set(expected["required_modules"]) - set(decision.modules))
        if missing:
            problems.append("missing modules: " + ", ".join(missing))
        if decision.risk != expected["risk"]:
            problems.append(f"risk expected {expected['risk']} got {decision.risk}")
        if problems:
