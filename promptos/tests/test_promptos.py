...
class PromptOSTests(unittest.TestCase):
    def request(self, source: str, **overrides) -> FoundryRequest:
        payload = {"source_material": source, **overrides}
        return FoundryRequest.from_dict(payload)
    ...
    def test_13_consequential_mode_requires_specific_actions(self) -> None:
    ...
    def test_14_consequential_authority_is_exact(self) -> None:
        ...
        self.assertEqual(
            package["pir"]["permissions"]["authorized_consequential_actions"],
            ["publish"],
        )
        self.assertNotIn("purchase", package["pir"]["permissions"]["authorized_consequential_actions"])

    def test_15_corpus_has_120_cases_and_60_30_30_split(self) -> None:
        ...
        self.assertEqual(
            len({case["case_id"] for caseset in partitions.values() for case in caseset}),
            120,
        )

    def test_16_full_conformance_and_resources(self) -> None:
